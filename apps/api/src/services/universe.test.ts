...
const marketByTid = (tid: number) =>
  db.query<Record<string, unknown>>(`SELECT id, featured, requires_scrydex, scrydex_card_id, jpy FROM markets WHERE tcgplayer_id=$1`, [tid]).then((r) => r.rows[0]);
...
test('applyUniverse: creates entrants (hidden flag), re-features stays, un-features drop-outs', async () => {
  ...
  await seed([
    ...
    { tid: 5003, game: 'mtg', name: 'M3-JP', sxUsd: 150, sxPrices: rawJpy(22500) }, // JPY ≥$100 → jpy-floor → jpy=true
  ]);
  ...
  assert.deepEqual({ c: dry.toCreate, u: dry.toUpdate, d: dry.toUnfeature, created: dry.created }, { c: 2, u: 1, d: 1, created: 0 }, 'dry-run plans but writes nothing');
  ...
  assert.equal(res.created, 2); // 5002 (EN Scrydex-only) + 5003 (JP)
  ...
  assert.equal(m5002.jpy, false, 'EN Scrydex-only card is not flagged JPY');
  assert.equal((await marketByTid(5003)).jpy, true, 'JPY>$100 card flagged jpy=true');
