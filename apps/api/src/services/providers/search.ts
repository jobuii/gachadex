...
const SEARCH_FETCH = 100; // deep provider page so the price sort can surface a high-value match the provider's relevance order buries past the display page
...
export async function searchCatalog(
  ...
  if (hit && Date.now() - hit.at < SEARCH_TTL_MS) {
    ...
    if (via === 'scrydex') {
      ...
      const page = await clients.scrydex.searchCards(slug, { q: q.trim(), pageSize: SEARCH_FETCH }, 'search');
      const rate = await fx();
      candidates = page.data.map((c) => scrydexCandidate(c, rate));
    } else {
      const cards = (await clients.tcgpl.searchCards({ q: q.trim(), game, limit: SEARCH_FETCH }, 'search')).data;
      candidates = cards.map(tcgplCandidate);
    }
    // The provider returns by relevance (cheap commons first); sort by price DESC so a high-value match
    // (e.g. a $170 promo) surfaces instead of being buried under junk and cut off the display page.
  ...
  const top = candidates.slice(0, SEARCH_PAGE);
  ...
  const sxIds = top.filter((c) => c.scrydexCardId).map((c) => c.scrydexCardId!);
  const provIds = top.filter((c) => c.source === 'tcgpl').map((c) => c.providerCardId);
  const tids = [...new Set(top.map((c) => c.tcgplayerId).filter((t): t is number => t != null))].map(String);
  const existing =
